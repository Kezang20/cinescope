    ...
    const hero = el('div',{class:'hero'},
      ...
      el('div',{class:'meta-block'},
        ...
        el('div',{}, el('button',{class:'btn', onclick:()=> {
          toggleWatchlist({ id:data.id, media_type:type, title: title, poster_path: data.poster_path, overview: data.overview, release_date: release });
          updateWatchCount();
          showToast(`${title} watchlist updated`);
        }}, isInWatchlist({id:data.id, media_type:type}) ? 'In Watchlist' : 'Add to Watchlist'))
      )
    );
    DOM.app.appendChild(hero);
